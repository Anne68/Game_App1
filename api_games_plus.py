# model_manager.py - Gestion du modèle de recommandation
# Pipeline: TF-IDF (title+genres+platforms) -> SVD -> KMeans (8 clusters) -> KNN (NearestNeighbors)
...
class RecommendationModel:
    ...
    Méthodes principales :
      ...
      - save_model(), load_model(), get_metrics()
    """
    ...
    def __init__(self, model_version: str = "2.1.0"):
        ...
        # Données/features
        self.game_features: Optional[np.ndarray] = None          # SVD + features numériques
    ...
    def prepare_features(self, games: List[Dict]) -> pd.DataFrame:
        ...
        # -------- AJOUT : colonne price (pour filtrage budget) --------
        if "price" not in df:
            df["price"] = np.nan
        df["price"] = pd.to_numeric(df["price"], errors="coerce")  # pas de normalisation: uniquement filtre
    ...
    def train(self, games: List[Dict]) -> Dict:
        ...
        logger.info(
            f"Model trained. Features: {self.game_features.shape}, clusters: {len(np.unique(self.cluster_labels))}"
        )
        return {
            "status": "success",
    ...
    def _neighbors_for_index(self, idx: int, k: int) -> List[Tuple[int, float]]:
        ...
        distances, indices = self.knn.kneighbors(
            [self.game_features[idx]], n_neighbors=min(k + 1, len(self.game_features))
        )
    ...
    def recommend_with_filters(
        ...
        Stratégie:
          ...
          4) Calculer cosine_similarity avec les SEULS candidats
          5) Trier (par score ou prix) et retourner top-k

        Note: ne nécessite aucun réentraînement.
        """
        if not self.is_trained:
            raise ValueError("Model not trained")
    ...
    def _update_metrics(self, recs: List[Dict], k: int):
        ...
        if recs:
            avg_conf = float(np.mean([r["confidence"] for r in recs[:k] if "confidence" in r]))
